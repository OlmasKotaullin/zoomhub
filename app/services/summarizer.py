...
CONTEXT_LIMITS = {
    ...
    "groq": 8000,        # Free tier TPM=12K — ограничиваем чтобы не 413
    "claude": 100000,
...
async def generate_summary(transcript_text: str, provider_name: str | None = None) -> dict:
    ...
    from app.config import (
        GOOGLE_AI_API_KEY, ANTHROPIC_API_KEY, DEEPSEEK_API_KEY,
        OPENROUTER_API_KEY, GROQ_API_KEY, GIGACHAT_AUTH_KEY,
    )

    if provider_name:
        providers = [make_provider_by_name(provider_name)]
    else:
        # Для суммаризации используем лучший доступный провайдер
        if GOOGLE_AI_API_KEY:
            providers = [make_provider_by_name("gemini")]
        elif DEEPSEEK_API_KEY:
            providers = [make_provider_by_name("deepseek")]
        elif OPENROUTER_API_KEY:
            providers = [make_provider_by_name("openrouter")]
        elif GROQ_API_KEY:
            providers = [make_provider_by_name("groq")]
        elif ANTHROPIC_API_KEY:
            providers = [make_provider_by_name("claude")]
        elif GIGACHAT_AUTH_KEY:
            providers = [make_provider_by_name("gigachat")]
        else:
            provider = get_provider_for_text(len(transcript_text))
            providers = [provider]

    # Fallback chain — все доступные провайдеры по убыванию качества
    if not provider_name:
        ...
        for name, key in fallbacks:
            if name != primary and key:
                providers.append(make_provider_by_name(name))
