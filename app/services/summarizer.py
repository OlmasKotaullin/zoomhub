...
CONTEXT_LIMITS = {
    ...
    "gigachat": 25000,   # 32K токенов — до 1.5ч встречи
    "claude": 100000,
    "ollama": 12000,     # ~16K токенов при num_ctx=16384
}


async def generate_summary(transcript_text: str, provider_name: str | None = None, api_key: str | None = None) -> dict:
    ...
    Args:
        ...
        api_key: пользовательский API-ключ (если None — используется глобальный)

    Returns:
        {"tldr": str, "tasks": list, "topics": list, "insights": list, "raw_response": str}
    """
    if provider_name:
        provider = make_provider_by_name(provider_name, api_key=api_key)
    else:
